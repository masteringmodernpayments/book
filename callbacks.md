...
The first thing we should do is handle a dispute which fires when a customer initiates a chargeback, which if we get enough of Stripe will think about dropping our account. In response to a dispute we send an email to ourselves with all of the details. Disputes should be rare enough that we can deal with them individually just with an email:
