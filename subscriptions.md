...
So far in the example project we've only dealt with one-off transactions where the customer comes along and buys a product once and we basically never have to deal with them again. The majority of SaaS products aren't really like that, though. Most SaaS producks bill customers monthly for services, maybe with some kind of add-on system. The general flow is:
